...
class Measured::DynamicMeasurableTest < ActiveSupport::TestCase
  setup do
    @arcane = DynamicMagic.unit_system.unit_for!(:arcane)
    @fireball = DynamicMagic.unit_system.unit_for!(:fireball)
    @magic_missile = DynamicMagic.unit_system.unit_for!(:magic_missile)
    @magic = DynamicMagic.new(10, @magic_missile)
  end

  test "#initialize requires two params, the amount and the unit" do
    assert_nothing_raised do
      DynamicMagic.new(1, "fireball")
    end
  ...
  test "#initialize converts unit string and symbol to Unit instance" do
    assert_equal @arcane, DynamicMagic.new(1, "arcane").unit
    assert_equal @arcane, DynamicMagic.new(1, :arcane).unit
  end

  test "#initialize raises if it is an unknown unit" do
    assert_raises Measured::UnitError do
      DynamicMagic.new(1, "slash")
    end
  end

  test "#initialize keeps Rationals, BigDecimals, and Integers as-is" do
    assert_equal 10, DynamicMagic.new(10, :fire).value
    assert_equal Rational(1, 3), DynamicMagic.new(Rational(1, 3), :fire).value
    assert_equal BigDecimal("123.887788"), DynamicMagic.new(BigDecimal("123.887788"), :fire).value
  end

  test "#initialize converts floats and strings to BigDecimal and does not round large numbers" do
    assert_equal BigDecimal(9.1234572342342, 14), DynamicMagic.new(9.1234572342342, :fire).value
    assert_equal BigDecimal("9.1234572342342"), DynamicMagic.new("9.1234572342342", :fire).value
  end

  test "#initialize converts strings to Rational if they follow Rational pattern" do
    assert_equal Rational(1, 3), DynamicMagic.new("1/3", :fire).value
  end

  test "#initialize converts to the base unit" do
    assert_equal @fireball, DynamicMagic.new(1, :fire).unit
  end
  ...
  test ".parse raises on a single incorrect number" do
    exception = assert_raises(Measured::UnitError) do
      DynamicMagic.parse("1234")
    end
    assert_equal "Cannot parse measurement from '1234'", exception.message
  end

  test ".parse takes input with a space between" do
    assert_equal DynamicMagic.new(1, :arcane), DynamicMagic.parse("1 arcane")
  end

  test ".parse takes input without a space" do
    assert_equal DynamicMagic.new(99, :ice), DynamicMagic.parse("99ice")
  end

  test ".parse takes float with a space" do
    assert_equal DynamicMagic.new(12.345, :arcane), DynamicMagic.parse("12.345 arcane")
  end

  test ".parse takes float without a space" do
    assert_equal DynamicMagic.new(9.9, :magic_missile), DynamicMagic.parse("9.9magic_missile")
  end

  test ".parse truncates any space before and after" do
    assert_equal DynamicMagic.new(8765, :arcane), DynamicMagic.parse("   8765     arcane    ")
  end

  test ".parse raises with multiple periods in fractional numbers" do
    exception = assert_raises(Measured::UnitError) do
      DynamicMagic.parse("12.34.56 ice")
    end
    assert_equal "Cannot parse measurement from '12.34.56 ice'", exception.message
  end

  test ".parse parses negative numbers" do
    assert_equal DynamicMagic.new(-12.34, :arcane), DynamicMagic.parse("-12.34 arcane")
  end

  test ".parse parses rational numbers" do
    assert_equal DynamicMagic.new(1.5, :magic_missile), DynamicMagic.parse("3/2magic missile")
  end

  test ".parse raises on unknown unit" do
    exception = assert_raises(Measured::UnitError) do
      DynamicMagic.parse("1 fake")
    end
  ...
  test "#to_s outputs the number and the unit" do
    assert_equal "1 fireball", DynamicMagic.new("1", :fire).to_s
    assert_equal "10 fireball", DynamicMagic.new(10, :fire).to_s
    assert_equal "1.234 magic_missile", DynamicMagic.new("1.234", :magic_missile).to_s
    assert_equal "0.125 magic_missile", DynamicMagic.new(Rational(1, 8), :magic_missile).to_s
    assert_equal "0.375 magic_missile", DynamicMagic.new(Rational(3, 8), :magic_missile).to_s
    assert_equal "5 fireball", DynamicMagic.new(Rational(5, 1), :fire).to_s
  end

  test "#to_s outputs the correct number of decimals" do
    assert_equal "9.3 fireball", DynamicMagic.new(9.3, :fire).to_s
  end

  test "#format outputs a formatted string" do
    measured_object = DynamicMagic.new(9.342, :magic_missile)
  ...
  test "#format with no formatting string uses the default one" do
    assert_equal "9.34 magic_missile", DynamicMagic.new(9.342, :magic_missile).format
  end

  test "#format can drop the conversion amount" do
    assert_equal "1.00 fireball (2/3 magic_missile)", DynamicMagic.new(1, :fireball).format
    assert_equal "1.00 fireball", DynamicMagic.new(1, :fireball).format(with_conversion_string: false)
  end

  test "#humanize outputs the number and the unit properly pluralized" do
    assert_equal "1 fireball", DynamicMagic.new("1", :fire).humanize
    assert_equal "10 fireballs", DynamicMagic.new(10, :fire).humanize
    assert_equal "1.234 magic_missiles", DynamicMagic.new("1.234", :magic_missile).humanize
    assert_equal "0.125 magic_missiles", DynamicMagic.new(Rational(1, 8), :magic_missile).humanize
    assert_equal "0.375 magic_missiles", DynamicMagic.new(Rational(3, 8), :magic_missile).humanize
    assert_equal "5 fireballs", DynamicMagic.new(Rational(5, 1), :fire).humanize
  end

  test "#inspect shows the number and the unit" do
    assert_equal "#<DynamicMagic: 10 #<Measured::Unit: fireball (fire, fireballs) 2/3 magic_missile>>", DynamicMagic.new(10, :fire).inspect
    assert_equal "#<DynamicMagic: 1.234 #<Measured::Unit: magic_missile (magic_missiles, magic missile)>>", DynamicMagic.new(1.234, :magic_missile).inspect
  end

  test "#<=> compares regardless of the unit" do
    assert_equal (-1), @magic <=> DynamicMagic.new(20, :fire)
    assert_equal 1, @magic <=> DynamicMagic.new(9, :magic_missile)
    assert_equal 0, @magic <=> DynamicMagic.new(Rational(30, 2), :fire)
    assert_equal (-1), @magic <=> DynamicMagic.new(11, :magic_missile)
  end
  ...
  test "#== should be the same if the classes, unit, and amount match" do
    ...
    assert_equal DynamicMagic.new(10, :magic_missile), DynamicMagic.new("10", "magic_missile")
    assert_equal DynamicMagic.new(1, :arcane), DynamicMagic.new(10, :magic_missile)
    refute_equal DynamicMagic.new(1, :arcane), DynamicMagic.new(10.1, :magic_missile)
  end

  test "#== should be the same if the classes and amount match but the unit does not so they convert" do
    assert_equal DynamicMagic.new(2, :magic_missile), DynamicMagic.new("1", "ice")
  end

  test "#== doesn't compare against zero" do
    arcane_zero = DynamicMagic.new(0, :arcane)
  ...
  test "#> and #< should compare measurements" do
    assert DynamicMagic.new(10, :magic_missile) < DynamicMagic.new(20, :magic_missile)
    refute DynamicMagic.new(10, :magic_missile) > DynamicMagic.new(20, :magic_missile)
  end

  test "#> and #< should compare measurements of different units" do
    assert DynamicMagic.new(10, :magic_missile) < DynamicMagic.new(100, :ice)
    refute DynamicMagic.new(10, :magic_missile) > DynamicMagic.new(100, :ice)
  end
